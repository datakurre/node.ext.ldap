    ...
    def load(self):
        ...
        if len(entry) != 1:
            raise RuntimeError(                            # pragma NO COVERAGE
                "Fatal. Expected entry does not exist "    # pragma NO COVERAGE
                "or more than one entry found"             # pragma NO COVERAGE
            )                                              # pragma NO COVERAGE
...
class LDAPStorage(OdictStorage):
    ...
    def __repr__(self):
        dn = self.DN.decode(
            'utf-8').encode('ascii', 'replace') or '(dn not set)'
        if self.parent is None:
            return "<%s - %s>" % (dn, self.changed)
        name = self.name.decode(
            'utf-8').encode('ascii', 'replace')
    ...
    def node_by_dn(self, dn, strict=False):
        ...
        for rdn in reversed(explode_dn(dn)):
            try:
                ...
                if strict:
                    raise ValueError('Tree contains no node by given DN. '
                                     'Failed at RDN {}'.format(rdn))
                return None
        return node
    ...
    def invalidate(self, key=None):
        ...
        if key is None:
            if self.changed:
                raise RuntimeError("Invalid tree state. Try to invalidate "
                                   "changed node.")
            self.storage.clear()
            self.attrs.load()
        ...
        try:
            ...
            if child.changed:
                raise RuntimeError(
                    "Invalid tree state. Try to invalidate "
                    "changed child node '%s'." % (key,))
            del self.storage[key]
        except KeyError:
            pass
    ...
    def _create_suitable_node(self, vessel):
        ...
        try:
            ...
            raise ValueError("No attributes found on vessel, cannot convert")
        node = LDAPNode()
